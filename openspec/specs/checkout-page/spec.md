...
The checkout page is the cart-to-order conversion flow. It allows authenticated users to select a shipping address from their saved addresses (or add a new one), pick a work-day shipping date, review their order summary, and complete a mock payment flow that creates an order in the system.
